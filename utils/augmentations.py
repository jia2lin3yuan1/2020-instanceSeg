...
class Resize(object):
    ...
    def __init__(self, cfg, resize_gt=True):
        ...
        self.discard_box_width = cfg.discard_box_width
        self.discard_box_height = cfg.discard_box_height
    ...
    def __call__(self, image, masks, boxes, labels=None):
        ...
        if self.resize_gt:
            ...
            boxes[:, [0, 2]] = boxes[:, [0,2]] * (float(width)  / float(img_w))
            boxes[:, [1, 3]] = boxes[:, [1,3]] * (float(height) / float(img_h))
...
class RandomSampleCrop(object):
    ...
    def __call__(self, image, masks, boxes=None, labels=None, min_ratio=0.6):
...
class BaseTransform(object):
    ...
    def __init__(self, cfg, mean=MEANS, std=STD, resize_gt=False):
        self.augment = Compose([
            ConvertFromInts(),
            Resize(cfg, resize_gt=resize_gt),
...
class FastBaseTransform(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        if self.preserve_aspect_ratio:
            _, h, w, _ = img.size()
            img_size = Resize.calc_size_preserve_ar(w, h, self.max_size)
            img_size = (img_size[1], img_size[0]) # Pytorch needs h, w
        else:
            img_size = (self.max_size, self.max_size)
