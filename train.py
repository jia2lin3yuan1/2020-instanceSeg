...
def plot_tfboard_figure(cfg, vis_imgs, vis_show=False, show_grad=False, max_vis=3):
    ...
    for k, ik in enumerate(vis_idxs):
        img = vis_imgs['rgb'][ik] #[3, ht, wd]
        tp  = vis_imgs['preds'][ik,0].cpu().detach().numpy() # [ht, wd]
        tg  = vis_imgs['gts'][ik, 0].cpu().detach().numpy() #[ch, ht, wd]
        tw  = vis_imgs['wghts'][ik,0].cpu().detach().numpy() #[ht, wd]

        # for multiple gpus
        if torch.cuda.device_count()>1:
            img, tp, tg, tw = img[0], tp[0], tg[0], tw[0]
        img = undo_image_transformation(cfg.backbone, img)
        ax[k, 0].imshow(img)
        ax[k, 1].imshow(tg)
...
def train(args, cfg, option, DataSet):
    ...
    if cfg.classify_en:
        param_grp.append({'params': net.refine_net.parameters(), 'lr': args.lr*option['refine_lr_alpha']})
    ...
    loss_keys = ['binary', 'pi', 'regul', 'quanty', 'rfn_cls', 'rfn_iou', 'rfn_seg'\
